...
import { Plus, AlertCircle, Clock, CheckCircle, Edit, Trash } from "lucide-react";
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setAllCases(userEmail ? cases.filter((c) => c.creatorEmail === userEmail) : []);
      } catch (error) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="bg-[#FFF0EE] border-l-4 border-[#C0392B] p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="text-[36px] font-bold text-[#C0392B]">{sufferingCases.length}</div>
          ...
          <Card className="bg-[#FFF4ED] border-l-4 border-[#E67E22] p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="text-[36px] font-bold text-[#E67E22]">{helpingCases.length}</div>
          ...
          <Card className="bg-[#F0FFF4] border-l-4 border-[#27AE60] p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="text-[36px] font-bold text-[#27AE60]">{resolvedCases.length}</div>
          ...
          <AccordionItem value="suffering" className="border-0">
            <AccordionTrigger className="bg-[#FFF0EE] hover:bg-[#FFE5E2] px-6 py-5 rounded-xl transition-colors">
              <div className="flex items-center gap-3">
                ...
                <span className="font-bold text-[#1C1C1E]">Cas en souffrance</span>
                <Badge className="bg-[#C0392B] text-white ml-2">{sufferingCases.length}</Badge>
              </div>
            </AccordionTrigger>
            <AccordionContent className="pt-4 space-y-3">
              {sufferingCases.map((c) => (
                <Card key={c.id} className="p-4 border-l-4 border-[#C0392B] bg-white">
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2 text-sm text-[#6B6B6B]">
                        <Badge className="bg-[#C0392B] text-white">Souffre encore</Badge>
                        <span>📍 {c.governorate}</span>
                        <span>📅 {new Date(c.dateSubmitted).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })}</span>
                      </div>
                      <Link to={`/cas/${c.id}`} className="font-semibold text-[#1C1C1E] hover:text-[#C0392B]">
                        {c.title}
                      </Link>
                    </div>
                    <div className="flex gap-2">
                      <Button size="sm" variant="outline" className="text-[#C0392B]">
          ...
          <AccordionItem value="helping" className="border-0">
            <AccordionTrigger className="bg-[#FFF4ED] hover:bg-[#FFEEE0] px-6 py-5 rounded-xl transition-colors">
              <div className="flex items-center gap-3">
                ...
                <span className="font-bold text-[#1C1C1E]">En cours d'aide</span>
                <Badge className="bg-[#E67E22] text-white ml-2">{helpingCases.length}</Badge>
              </div>
            </AccordionTrigger>
            <AccordionContent className="pt-4 space-y-3">
              {helpingCases.map((c) => (
                <Card key={c.id} className="p-4 border-l-4 border-[#E67E22] bg-white">
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2 text-sm text-[#6B6B6B]">
                        <Badge className="bg-[#E67E22] text-white">En cours d'aide</Badge>
                        <span>📍 {c.governorate}</span>
                        <span>📅 {new Date(c.dateSubmitted).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })}</span>
                      </div>
                      <Link to={`/cas/${c.id}`} className="font-semibold text-[#1C1C1E] hover:text-[#C0392B]">
              ...
              {resolvedCases.map((c) => (
                <Card key={c.id} className="p-4 border-l-4 border-[#27AE60] bg-white">
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2 text-sm text-[#6B6B6B]">
                        <Badge className="bg-[#27AE60] text-white">Résolu</Badge>
                        <span>📍 {c.governorate}</span>
                        <span>📅 {new Date(c.dateSubmitted).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })}</span>
                      </div>
                      <Link to={`/cas/${c.id}`} className="font-semibold text-[#1C1C1E] hover:text-[#C0392B]">
                    ...
                    <Link to={`/cas/${c.id}`}>
                      <Button size="sm" variant="outline" className="text-[#27AE60]">
                        Voir la résolution →
                      </Button>
                    </Link>
                  </div>
