...
export function AdminModeration() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setPendingCases(await fetchCases({ status: "SUFFERING", limit: 50 }));
      } catch (error) {
    ...
    <AdminLayout>
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-4">
          <h2 className="text-xl font-bold text-[#1C1C1E]">File de modération</h2>
          <Badge className="bg-[#C0392B] text-white">{pendingCases.length} cas en attente</Badge>
        </div>
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Titre du cas</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {pendingCases.map((caseData, index) => (
                <tr key={caseData.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-600">{caseData.creatorEmail}</td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex gap-2">
                      ...
                      <Button size="sm" className="bg-[#27AE60] hover:bg-[#229954] text-white">
