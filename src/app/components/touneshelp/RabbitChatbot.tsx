...
const knowledgeBase = {
  fr: {
    ...
    medical: {
      response: "Les cas médicaux sont disponibles dans la liste des cas avec filtrage par catégorie.",
      links: [{ text: "Voir cas médicaux", url: "/?category=Médical" }]
    },
    education: {
      response: "Les cas éducatifs sont disponibles dans la liste des cas avec filtrage par catégorie.",
...
function getBotResponse(userMessage: string, language: Language, cases: TunisiaCase[]): { response: string; links?: { text: string; url: string }[] } {
  ...
  if (normalizedMessage.includes('inscr') || normalizedMessage.includes('سجل') || normalizedMessage.includes('register') || normalizedMessage.includes('compte') || normalizedMessage.includes('حساب')) {
    return kb.subscribe;
  }

  // Check for case creation
  if (normalizedMessage.includes('créer') || normalizedMessage.includes('cas') || normalizedMessage.includes('أنشئ') || normalizedMessage.includes('حالة') || normalizedMessage.includes('create') || normalizedMessage.includes('case') || normalizedMessage.includes('signaler')) {
    return kb.createCase;
  }

  // Check for active cases / statistics
  if (normalizedMessage.includes('actif') || normalizedMessage.includes('voir') || normalizedMessage.includes('نشط') || normalizedMessage.includes('عرض') || normalizedMessage.includes('active') || normalizedMessage.includes('view') || normalizedMessage.includes('statist')) {
  ...
  const caseIdMatch = normalizedMessage.match(/#(\d+)|cas (\d+)|حالة (\d+)|case (\d+)/);
  if (caseIdMatch) {
    const caseId = caseIdMatch[1] || caseIdMatch[2] || caseIdMatch[3] || caseIdMatch[4];
    const caseData = cases.find(c => c.id === caseId);
    
    if (caseData) {
      const statusEmoji = caseData.status === 'suffering' ? '🔴' : caseData.status === 'helping' ? '🟠' : '🟢';
      const statusText = language === 'fr' 
        ? (caseData.status === 'suffering' ? 'Souffre encore' : caseData.status === 'helping' ? "En cours d'aide" : 'Résolu')
        : language === 'ar'
        ? (caseData.status === 'suffering' ? 'لا يزال يعاني' : caseData.status === 'helping' ? 'قيد المساعدة' : 'محلول')
        : (caseData.status === 'suffering' ? 'Still suffering' : caseData.status === 'helping' ? 'Being helped' : 'Resolved');
      ...
      return {
        response,
        links: [{ text: language === 'fr' ? 'Voir le cas' : language === 'ar' ? 'عرض الحالة' : 'View case', url: `/request/${caseId}` }]
      };
    }
  }

  // Check for medical cases
  if (normalizedMessage.includes('médical') || normalizedMessage.includes('medical') || normalizedMessage.includes('طبي') || normalizedMessage.includes('santé') || normalizedMessage.includes('صحة')) {
    return kb.activeCases;
  }

  // Check for education cases
  if (normalizedMessage.includes('éducation') || normalizedMessage.includes('education') || normalizedMessage.includes('تعليم') || normalizedMessage.includes('école') || normalizedMessage.includes('مدرسة')) {
    return kb.activeCases;
  }
    ...
    <Card className="fixed bottom-6 right-6 w-[400px] h-[600px] shadow-2xl rounded-3xl z-50 bg-white border-4 border-pink-200 flex flex-col overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-pink-400 via-pink-500 to-pink-600 text-white p-4 flex items-center justify-between relative overflow-hidden">
        ...
        <div className="flex items-center gap-2 relative z-10">
          ...
          <div className="relative group">
            <button className="hover:bg-white/20 p-2 rounded-full transition-colors" title="Choisir la langue">
              <Globe size={18} />
            </button>
      ...
      {messages.length === 1 && (
        <div className="p-4 bg-pink-50 border-b border-pink-100">
          <p className="text-xs text-pink-700 mb-2 font-medium">{t.suggestionsTitle}</p>
      ...
      {/* Messages Area */}
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gradient-to-b from-pink-50/30 to-white" dir={language === 'ar' ? 'rtl' : 'ltr'}>
          ...
          >
            ...
            >
              ...
              <span className="text-xs text-gray-500 px-1">
                {message.timestamp.toLocaleTimeString(language === 'ar' ? 'ar-TN' : 'fr-FR', { hour: '2-digit', minute: '2-digit' })}
              </span>
            </div>
          </div>
        ))}
